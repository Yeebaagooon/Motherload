...
void PaintSellTerrain (int x =0, int z = 0){
	...
	trPaintTerrain(x+3, z-2, x+3, z-2, 0, 77, false);
...
void PaintHullTerrain (int x =0, int z = 0){
	//row one
	trPaintTerrain(x, z, x, z, 0, 76, false);
	trPaintTerrain(x+1, z, x+1, z, 0, 76, false);
	trPaintTerrain(x+2, z, x+2, z, 0, 76, false);
	trPaintTerrain(x+3, z, x+3, z, 0, 76, false);
	trPaintTerrain(x+4, z, x+4, z, 0, 76, false);
	//null
	...
	//row two
	trPaintTerrain(x, z-1, x, z-1, 0, 79, false);
	trPaintTerrain(x+1, z-1, x+1, z-1, 0, 77, false);
	trPaintTerrain(x+2, z-1, x+2, z-1, 0, 74, false);
	trPaintTerrain(x+3, z-1, x+3, z-1, 0, 74, false);
	trPaintTerrain(x+4, z-1, x+4, z-1, 0, 74, false);
	...
	//row three
	trPaintTerrain(x, z-2, x, z-2, 0, 78, false);
	trPaintTerrain(x+1, z-2, x+2, z-2, 0, 78, false);
	trPaintTerrain(x+2, z-2, x+3, z-2, 0, 82, false);
	trPaintTerrain(x+3, z-2, x+4, z-2, 0, 81, false);
	trPaintTerrain(x+4, z-2, x+4, z-2, 0, 82, false);
	trPaintTerrain(x+5, z-2, x+5, z-2, 0, 77, false);
	trPaintTerrain(x+6, z-2, x+6, z-2, 0, 82, false);
	trPaintTerrain(x+7, z-2, x+7, z-2, 0, 77, false);
...
void Trap3Vertical(int LZx = 0, int LZZmin = 0, int LZZmax = 0, int PPXmin = 0, int PPXmax = 0, int PPZmin = 0, int PPZmax = 0, int Timer = 0){
	int temp = 0;
	trPaintTerrain(LZx,LZZmin,LZx,LZZmax,0,71,false);
	//gate effect
	xAddDatabaseBlock(dTrap);
	xSetPointer(dTrap, xGetDatabaseCount(dTrap));
	//pressure pad
...
void Trap2(int PPXmin = 0, int PPXmax = 0, int PPZmin = 0, int PPZmax = 0, int WallX = 0, int WallZ = 0, vector Vtarget = vector(0,0,0), int Timer = 20000){
	...
	xSetBool(dTrap, xTrapReady, true);
	xSetInt(dTrap, xTrapReset, Timer);
	xSetVector(dTrap, xTrapStartVector, xsVectorSet(WallX,3,WallZ));
	//Direction
	xSetVector(dTrap, xTrapTargetVector, Vtarget);
	xSetInt(dTrap, xTrapUnit, 1*trGetNextUnitScenarioNameNumber());
	temp = trGetNextUnitScenarioNameNumber();
	trArmyDispatch(""+cNumberNonGaiaPlayers+",0", "Dwarf", 1, WallX,3,WallZ, 0, true);
	trUnitSelectClear();
	trUnitSelect(""+temp);
...
void Trap1(int PPXmin = 0, int PPZmin = 0, int PPXmax = 0, int PPZmax = 0, vector VTarget = vector(0,0,0), int Timer = 5000, int DEBUG = 0){
	int temp = 0;
	trPaintTerrain(PPXmin,PPZmin,PPXmax,PPZmax,0,73,false);
	xAddDatabaseBlock(dTrap);
	xSetPointer(dTrap, xGetDatabaseCount(dTrap));
	...
	if(DEBUG == 1){
		trUnitSelectClear();
		trUnitSelect(""+temp);
		trUnitTeleport(xsVectorGetX(VTarget), 3, xsVectorGetZ(VTarget));
	}
	if((PPXmax - PPXmin == 2) && (PPZmax - PPZmin == 2)){
		PaintAtlantisArea(PPXmin,PPZmin,PPXmax,PPZmax,"black");
